...
async function page({ params }: PageProps) {
  ...
  const aurl = process.env.AUTH_SERVER_URL!
  ...
  const {data} = await axios.post(`${aurl}/getstocktimeserise`, {code : code});
  ...
  return (
    <div className="flex items-center    space-y-5 flex-col h-screen">
      ...
      <div className="flex flex-wrap  space-y-2 md:space-y-0 justify-evenly  p-4  max-w-6xl w-full mx-auto">
        <div className="w-[500px] items-center  flex flex-col space-y-7  h-[500px] p-2 ">
          {/* this part is used to show the interval data */}
          {/* <ChartSmall
            name={s}
            XplaneVals={d_interval.slice(-50)}
            YPlaneValsClose={cls_interval.slice(-50)}
            borderColor="black"
          /> */}
        </div>
      ...
      <div className="w-[400px]">
        <UserFetched mode={mode} stockName={s} code = {code} backendurl={backendurl!} />
      </div>
