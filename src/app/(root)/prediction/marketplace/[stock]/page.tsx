...
const ResizeableFileTracker = dynamic(
  () => import("@/src/app/components/ResizeableFileTracker"),
  {
    ssr: false,
  }
);
...
async function page({ params }: PageProps) {
  ...
  //const {data} = await axios.post(`${furl}/api/getMarket`, {code : code});
  const [date, value] = dateandvalues(data.results);
  //const {data : data_interval} = await axios.post(`${furl}/api/getMarketInterval`, {code : code});
  ...
  return (
    <div className="flex    space-y-5 flex-col h-screen">
      ...
      <div className=" w-[440px]   h-[500px]">
          <ChartMarket name={s}  XplaneVals={date.slice(-50)}
            YPlaneValsClose={cls.slice(-50)}  borderColor="red" />
      </div>

      <div className="flex flex-wrap  space-y-2 md:space-y-0 justify-evenly  p-4  max-w-6xl w-full mx-auto">
        <div className="w-[400px] items-center  flex flex-col space-y-7  h-[500px] p-2 ">
          {/* this part is used to show the interval data */}
          <ChartSmall
            name={s}
            XplaneVals={date.slice(-50)}
            YPlaneValsClose={cls.slice(-50)}
            borderColor="black"
          />
        </div>
      ...
      <div className="w-[400px]">
        <UserFetched mode={mode} stockName={s} backendurl={backendurl!} />
      </div>
          ...
          <div className="container px-6 py-12 mx-auto">
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-y-10 lg:grid-cols-4">
              <div className="sm:col-span-2">
                <h1 className="max-w-lg text-xl font-semibold tracking-tight text-gray-900">
              ...
              <div>
                ...
                <div className="flex flex-col items-start mt-5 space-y-2">
                  <p className=" transition-colors duration-300 text-black hover:underline hover:cursor-pointer hover:text-blue-500">
                    Home
                  </p>
                  <p className="text-black transition-colors duration-300  dark:hover:text-blue-400 hover:underline hover:cursor-pointer hover:text-blue-500">
                    Who We Are
                  </p>
                  <p className="text-black transition-colors duration-300  dark:hover:text-blue-400 hover:underline hover:cursor-pointer hover:text-blue-500">
                    Our Philosophy
            ...
            <p className="font-sans p-8  text-black text-start md:text-center md:text-lg md:p-4">
