...
def create_sequences(data, time_steps):
    ...
    for i in range(len(data) - time_steps):
        X.append(data[i:(i + time_steps), :-1])  # Use 'High', 'Low' for features
        y.append(data[i + time_steps, -1])  # Use 'Close' as target
    return np.array(X), np.array(y)


def LSTMAlgorithmTwo(fileName, epochs):
    ...
    model.save('pretad/' + fileName + ".h5")

    return model


def LSTMAlgorithm(fileName, userId, train_size, epochs, updateEpochs):
    filepath = f"./datasets/{userId}/{fileName}.csv"
    ...
    print("Saving Model--------------------------------------------->")
    savepath  = f"pretrained/{userId}/{fileName}.h5"
    model.save(savepath)

    return model


def getPredictonsFromModelAd(filename, train_size):
    df = pd.read_csv('./datasets/' + filename + '.csv')
    features = ['High', 'Low', 'Close']
    target = ['Close']
    ...
    model = tf.keras.models.load_model('./pretad/' + filename + '.h5')
    predictions = model.predict(X_test)

    combined_array = np.concatenate((X_test[:, -1, :], predictions), axis=1)
    # print(combined_array.shape)
    combined_array_inverse = scaler.inverse_transform(combined_array)

    # Extract the predicted close prices from the combined array
    predicted_close_prices = combined_array_inverse[:, -1]
    ...
    y_test = y_test[:, -1]
    y_test = list(y_test)
    predicted_close_prices = list(predicted_close_prices)

    aa = predicted_close_prices
    bb = y_test

    dates = df["Date"].values

    n = len(predicted_close_prices) - 1
    accuracy = 0
    for i in range(n):
        accuracy += (abs(y_test[i] - predicted_close_prices[i]) / y_test[i]) * 100
    ...
    json = {
        "predictions": {
            "dates": list(dates[0:len(y_test) - 1]),
            "realData": y_test,
            "predictedData": predicted_close_prices,
...
def getPredictonsFromModel(fileName, userId,  train_size):
    filepath = f"./datasets/{userId}/{fileName}.csv"
    df = pd.read_csv(filepath)
    cols, dateColName, trade_close_col = getRequiredColumns(df)
    modelpath = f"./pretrained/{userId}/{fileName}.h5"
    ...
    pred = [round(number, 2) for number in pred]
    print(list(pred))
    n = len(pred)
    ...
    for i in range(len(predictionData)): predictionData[i] = round(float(predictionData[i]),2)
...
def getManualPredictionForModel(fileName, train_size, openValue, highValue, lowValue, volumeValue):
    ...
    for i in range(df.shape[1]):
        if (i == close_idx):
            row.append(random.randint(int(float(lowValue)), int(float(highValue))))
        else:
            row.append(0)
